...
function registerStudent(user, callback){
  let query = `INSERT INTO students (name, email, phone, college, year, department, degree, graduation_year, dob, password) 
    VALUES (?,?,?,?,?,?,?,?,?,?)`;
  let values = [user.name, user.email, user.phone, user.college, user.year, user.department,
     user.degree, user['graduation-year'], user.dob, user.password1];
